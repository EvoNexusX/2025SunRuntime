%直接将该问题将这些点划分为10个终点，1-5为1,6-10为2

%该问题出现点的重复没关系，因为这样算出来的目标值也是0，但是出现环就不太行了

%解x中只有轨迹点组成的这条路径，高度是随机生成的

%% four objectives
%目标1：最小化路径长度可以降低UAV的飞行时间和消耗
%目标2：最小化UAV路径与目标点之间的距离，确保路径覆盖尽可能靠近目标设备
%目标3：最小化死亡风险，确保路径经过区域的安全性——相同trace下h越小，死亡风险越小
%目标4：最小化财务成本，优化UAV的经济性——h越大，每个点的财务风险越小

%针对于优化目标1和目标3，可以很快的得到每一方的PF中的解，从而得到公共解。


% 公共解：既是PF1,又是PF2。他在两方目标值都不被支配
% 基本上公共解就是1,2   1 3 (不会是1,2,3 因为h=0的时候是个平面，两边之和大于第三边，所以中间插任何点，最后结果都会变大)，但是在前两个目标上面，轨迹点离得越近越好
% 然后对于高度的话就是高度为0就一定能满足他是PF中的解


%% 
classdef UAV1 < handle
        %% Properties  
        ...
        function obj = UAV1(seed)
            ...
            obj.common = obj.data.common1; %顶点数量为10时的公共解
        end
